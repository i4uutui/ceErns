...
router.get('/process_code', authMiddleware, async (req, res) => {
  ...
  // 查询总记录数
  const [countRows] = await pool.execute(
    `SELECT COUNT(*) as total FROM sub_process_code WHERE user_id IN (${placeholders}) AND is_delete = 0`,
    countParams
  );
...
router.post('/process_code', authMiddleware, async (req, res) => {
  ...
  const [result] = await pool.execute(
    'INSERT INTO sub_process_code (process_code, process_name, equipment_used, piece_working_hours, processing_unit_price, section_points, total_processing_price, remarks, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [process_code, process_name, equipment_used, piece_working_hours, processing_unit_price, section_points, total_processing_price, remarks, userId]
  );
...
router.put('/process_code', authMiddleware, async (req, res) => {
  ...
  // 更新工艺编码信息
  const [updateResult] = await pool.execute(
    'UPDATE sub_process_code SET process_code = ?, process_name = ?, equipment_used = ?, piece_working_hours = ?, processing_unit_price = ?, section_points = ?, total_processing_price = ?, remarks = ?, user_id = ? WHERE id = ?',
    [process_code, process_name, equipment_used, piece_working_hours, processing_unit_price, section_points, total_processing_price, remarks, userId, id]
  );
...
router.delete('/process_code/:id', authMiddleware, async (req, res) => {
  ...
  // 更新 deleted_at 为当前时间
  const [result] = await pool.execute(
    'UPDATE sub_process_code SET is_delete = 1 WHERE id = ? AND is_delete = 0',
    [id]
  );
...
router.get('/equipment_code', authMiddleware, async (req, res) => {
  ...
  // 查询总记录数
  const [countRows] = await pool.execute(
    `SELECT COUNT(*) as total FROM sub_equipment_code WHERE user_id IN (${placeholders}) AND is_delete = 0`,
    countParams
  );
  const total = countRows[0].total;
...
router.post('/equipment_code', authMiddleware, async (req, res) => {
  ...
  const [result] = await pool.execute(
    'INSERT INTO sub_equipment_code (equipment_code, equipment_name, equipment_quantity, department, working_hours, equipment_efficiency, equipment_status, remarks, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [equipment_code, equipment_name, equipment_quantity, department, working_hours, equipment_efficiency, equipment_status, remarks, userId]
  );
...
router.put('/equipment_code', authMiddleware, async (req, res) => {
  ...
  // 更新设备信息
  const [updateResult] = await pool.execute(
    'UPDATE sub_equipment_code SET equipment_code = ?, equipment_name = ?, equipment_quantity = ?, department = ?, working_hours = ?, equipment_efficiency = ?, equipment_status = ?, remarks = ?, user_id = ? WHERE id = ?',
    [equipment_code, equipment_name, equipment_quantity, department, working_hours, equipment_efficiency, equipment_status, remarks, userId, id]
  );
...
router.delete('/equipment_code/:id', authMiddleware, async (req, res) => {
  ...
  // 更新 deleted_at 为当前时间
  const [result] = await pool.execute(
    'UPDATE sub_equipment_code SET is_delete = 1 WHERE id = ? AND is_delete = 0',
    [id]
  );
...
router.get('/employee_info', authMiddleware, async (req, res) => {
  ...
  // 查询总记录数
  const [countRows] = await pool.execute(
    `SELECT COUNT(*) as total FROM sub_employee_info WHERE user_id IN (${placeholders}) AND is_delete = 0`,
    countParams
  );
  const total = countRows[0].total;
...
router.post('/employee_info', authMiddleware, async (req, res) => {
  ...
  const [result] = await pool.execute(
    'INSERT INTO sub_employee_info (employee_id, name, department, production_position, salary_attribute, remarks, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [employee_id, name, department, production_position, salary_attribute, remarks, userId]
  );
...
router.put('/employee_info', authMiddleware, async (req, res) => {
  ...
  try {
    // 更新员工信息
    const [updateResult] = await pool.execute(
      'UPDATE sub_employee_info SET employee_id = ?, name = ?, department = ?, production_position = ?, salary_attribute = ?, remarks = ?, user_id = ? WHERE id = ?',
      [employee_id, name, department, production_position, salary_attribute, remarks, userId, id]
    );
...
// 删除员工信息
router.delete('/employee_code/:id', authMiddleware, async (req, res) => {
  const { id } = req.params;
  // 更新 deleted_at 为当前时间
  const [result] = await pool.execute(
    'UPDATE sub_employee_info SET is_delete = 1 WHERE id = ? AND is_delete = 0',
    [id]
  );
